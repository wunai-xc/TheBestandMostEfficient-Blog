...
`.post-card` / `.post-nav a` / `.friend-card` / `.changelog-list` 的四个角有一道描边：顶点颜色最深，沿边向外逐渐变淡、变细。

- 实现：一个伪元素叠 **16 层 `background`**（4 角 × 2 边 × 2 粗细）画完，不增加任何 DOM。token 全在 `:root` 的 `--cn-*`（颜色取 `--accent`，亮/暗自动跟随）。
- 每个角每条边叠两层：**短而粗**（先列 = 在上）+ **长而细**（后列 = 在下），超出粗线范围只剩细线，这才是“向外变细”；颜色淡出由 `linear-gradient` 到 `transparent` 完成。
- 顶点处横竖两条线重叠，自然成为最浓的一点，无需额外提亮。
- 位置用百分比对齐（`0` / `100%`），卡片尺寸怎么变都贴角。
- **伪元素分配**：`.post-card` 的 `::after` 已被亚克力层占用，所以它用 `::before`；其余三个用 `::after`。
- **层级**：角标层为 `z-index: 1`。亚克力/光泽伪元素是 `z-index: 0` 且晚于 `::before` 绘制，若不提升就会被半透底盖得很淡；卡片内容也是 `z-index: 1` 但晚于伪元素，所以文字仍在角标之上。
- 打印时全部 `display: none`（装饰而已，省墨）。

调参：`--cn-len-thick`（粗线长）/ `--cn-len-thin`（细线长）/ `--cn-w-thick` / `--cn-w-thin`，整体浓度改伪元素上的 `opacity`。
