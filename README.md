...
- 角标：带边框的面板与按钮内侧画一个暗红细线矩形框，只在左上、右下两角额外画十字（右上、左下仅作为框角）；覆盖文章卡片、上下篇、友链卡片、最近更新面板、正文阅读面、分页、标签、打印按钮、回到顶部等，内缩与臂长按尺度分两级。`<input>`（.search-box）因伪元素不渲染、代码块与表格因自身横向滚动而排除
