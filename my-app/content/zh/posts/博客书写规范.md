...
带边框的面板与按钮，在**四角内侧**各有一枚小金十字：两条短臂交叉，交点最浓，沿臂向两端逐渐变淡、变细，发丝级 1px 金色线条。

**覆盖范围**

| 尺度 | 元素 |
|------|------|
| 面板（内缩 13px / 臂 4.5px） | `.post-card`、`.post-nav a`、`.friend-card`、`.changelog-list`、`.post-content`（正文阅读面）、`.collection-card`、`.toc-panel` |
| 小元件（内缩 6px / 臂 2.5px） | `.pagination a/span`、`.term-item`、`.footer-link`、`.print-btn`、`.tag`、`.author-badge`、`.back-to-top`、`.toc-fab` |
...
- 一个伪元素叠 **8 层 `background`**（4 角 × 2 臂）画完，不增加任何 DOM。比上一版的 16 层少一半，光栅化开销也低一半。
- 每条臂是「透明 → 金 → 透明」的**对称渐变**：中段最浓、两端淡出；两条臂在中心交叠，自然形成最浓的交叉点（不需额外画一个点）。
- **定位**：`--cn-a = inset - arm`（沿臂方向距边），`--cn-b = inset - w/2`（垂直方向居中）；右侧/下侧用 `calc(100% - …)` 镜像。用 calc + 百分比而不是固定 `inset`，元素尺寸怎么变都跟得上。
- **颜色**：`--cn-cross` 亮色主题用深金 `#b08d3f`（白底上才看得清），暗色换浅金 `#e0c383`。
- **内缩**：`--cn-inset` 让十字离开边框，不再压在描边上。
- **尺寸分级**：小元件必须同时收 `--cn-inset` 与 `--cn-arm`，否则 13px 的内缩在 20px 高的标签上会把十字挤到中间。
- **伪元素分配**：`.post-card` 的 `::after` 已被亚克力层占用，所以它用 `::before`；其余均用 `::after`。
- **层级**：`z-index: 1`。亚克力/光泽伪元素是 `z-index: 0` 且晚于 `::before` 绘制，不提升会被半透底盖淡；内容也是 `z-index: 1` 但晚于伪元素，文字仍在十字之上。
- 打印时全部 `display: none`（装饰，省墨）。

**注意**：`.post-content` 的内容自 `z-index: 1`，而 `::after` 也在 `z-index: 1` 且更晚绘制，因此十字会压在内容之上；它只占四角 9px 内的小区域，正文内边距 16px，重叠极小。若觉得碍眼，把它的 `--cn-inset` 调大或单独把 `.post-content::after` 去掉。

调参：`--cn-cross`（金色）/ `--cn-inset`（内缩）/ `--cn-arm`（臂长）/ `--cn-w`（线宽），整体浓度改伪元素上的 `opacity`（默认 0.8）。
