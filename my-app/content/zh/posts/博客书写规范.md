...
带边框的面板与按钮，在其内侧画一个**内缩的矩形框**（四条边连起来），并只在**左上、右下两角**额外画十字；右上与左下只是框线的拐角。颜色为暗红，**所有线都带渐变**（两端淡出）。

**关于“线段 / 射线 / 直线”**

CSS 只能画有界的色块，因此画出来的一律是**线段**（两端点）；真正的**直线**（两端无限）与**射线**（一端无限）无法表达。让线段读起来像直线的手段就是**两端淡出到透明** —— 淡出暗示线还在继续。所以：

- 十字的臂用 `--cn-h / --cn-v`：中段实、**两端完全淡出**，四条臂近似两条穿过交点的无限长线
- 框边用 `--cn-eh / --cn-ev`：中段保持实色、**只在靠近拐角处淡出**。若四边也完全淡出，角落就断了、不成为框
...
- **四组渐变而非实色**：`--cn-h / --cn-v`（十字臂，两端完全淡出）、`--cn-eh / --cn-ev`（框边，靠近拐角处淡出）；另有 `--cn-solid` 备用，想要不带渐变的实线（线段）时可用。
