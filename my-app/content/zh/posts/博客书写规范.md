...
带边框的面板与按钮四角都有一道描边：顶点颜色最深，沿边向外逐渐变淡、变细。

**覆盖范围**

| 尺度 | 元素 |
|------|------|
| 面板（`--cn-scale: 1`，默认） | `.post-card`、`.post-nav a`、`.friend-card`、`.changelog-list`、`.post-content`（正文阅读面）、`.collection-card`、`.toc-panel` |
| 小元件（`--cn-scale: 0.45`） | `.pagination a/span`、`.term-item`、`.footer-link`、`.print-btn`、`.tag`、`.author-badge`、`.back-to-top`、`.toc-fab` |
...
- 一个伪元素叠 **16 层 `background`**（4 角 × 2 边 × 2 粗细）画完，不增加任何 DOM。token 全在 `:root` 的 `--cn-*`（颜色取 `--accent`，亮/暗自动跟随）。
- 每个角每条边叠两层：**短而粗**（先列 = 在上）+ **长而细**（后列 = 在下），超出粗线范围只剩细线，这才是“向外变细”；颜色淡出由 `linear-gradient` 到 `transparent` 完成。
- 顶点处横竖两条线重叠，自然成为最浓的一点，无需额外提亮。
- 位置用百分比对齐（`0` / `100%`），元素尺寸怎么变都贴角。
- **尺寸缩放**：线长用 `calc(… * var(--cn-scale, 1))` 计算。小元件必须调小，否则 72px 的细线远长于元素本身、被裁成一根贴边的实线。
- **伪元素分配**：`.post-card` 的 `::after` 已被亚克力层占用，所以它用 `::before`；其余均用 `::after`。
- **层级**：角标层为 `z-index: 1`。亚克力/光泽伪元素是 `z-index: 0` 且晚于 `::before` 绘制，若不提升就会被半透底盖得很淡；内容也是 `z-index: 1` 但晚于伪元素，所以文字仍在角标之上。
- 打印时全部 `display: none`（装饰而已，省墨）。

**代价**：每个元素 16 层渐变背景。文章列表页一屏十几张卡片加上标签/分页，可能到二三百层，**移动端首帧与滚动会多出可观的光栅化开销**。若发现卡顿，把细线那 8 层（`--cn-*2`）去掉只留“粗线 + 顶点”，能立刻减半。

调参：`--cn-len-thick`（粗线长）/ `--cn-len-thin`（细线长）/ `--cn-w-thick` / `--cn-w-thin`，整体浓度改伪元素上的 `opacity`，单类元素改它的 `--cn-scale`。
