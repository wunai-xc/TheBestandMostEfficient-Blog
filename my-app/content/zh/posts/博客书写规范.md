...
带边框的面板与按钮，在其内侧画一个**内缩的矩形框**（四条边连起来），并只在**左上、右下两角**额外画十字；右上与左下只是框线的拐角。颜色为暗红。

**颜色分布：全图只有两个交点最深**

整张图上只有左上、右下两个十字交点颜色最深，**所有线条从交点出发向外连续变淡，全程没有恒定色段**：

- 十字的 4 条臂：`--cn-h / --cn-v`，以交点为最浓的对称渐变（向两端淡出）
- 框的 4 条边：`--cn-e-tl-r / --cn-e-tl-d`（上边与左边，从左上交点向右/向下淡出）、`--cn-e-br-l / --cn-e-br-u`（下边与右边，从右下交点向左/向上淡出），各自是单向渐变

> 早先的写法是“两端淡出、中间固定色”，结果边线中段一直是满色，就不是“只有交点最深”了 —— 上面这四组单向渐变才是正确做法。

**关于“线段 / 射线 / 直线”**

CSS 只能画有界的色块，因此画出来的一律是**线段**（两端点）；真正的**直线**（两端无限）与**射线**（一端无限）无法表达。让线段读起来像直线的手段就是**端点淡出到透明** —— 淡出暗示线还在继续（所以十字的臂两端都淡出，看起来像一条穿过交点的无限长线）。

**覆盖范围**

| 尺度 | 元素 |
|------|------|
| 面板（内缩 5px / 臂 3.5px） | `.post-card`、`.post-nav a`、`.friend-card`、`.changelog-list`、`.post-content`（正文阅读面）、`.collection-card`、`.toc-panel` |
| 小元件（内缩 4px / 臂 2px） | `.pagination a/span`、`.term-item`、`.footer-link`、`.print-btn`、`.tag`、`.author-badge`、`.back-to-top`、`.toc-fab` |

小元件之所以要分一档：5px 的内缩落在 20px 高的标签上，框只剩 10px 高，会被挤到中间。
...
- **四组单向/对称渐变，没有任何恒定色段**：`--cn-h / --cn-v`（十字臂，中心最浓、两端淡出）、`--cn-e-tl-* / --cn-e-br-*`（框边，各从交点角单向淡出）；另有 `--cn-solid` 备用，想要无渐变的实线时可用。
...
- **颜色** `--cn-color`：亮色主题 `#7f1d1d`（暗红），暗色主题 `#a63a34`（略亮一档 —— 真正的暗红在 `#0e0e10` 上几乎看不见）。
- **尺寸分级**：小元件必须同时收 `--cn-inset` 与 `--cn-arm`，否则 5px 的内缩在 20px 高的标签上会把框挤到中间。
- **内缩与臂长要联动**：`--cn-arm` 决定十字伸出框线多远。内缩 5px 时若臂长仍为 4.5px，臂尖会顶到元素自己的边框上（只剩 0.5px），所以同步收到 3.5px。
...
调参：`--cn-color`（颜色）/ `--cn-inset`（内缩）/ `--cn-arm`（臂长）/ `--cn-w`（线宽），整体浓度改伪元素上的 `opacity`（默认 0.6）。
