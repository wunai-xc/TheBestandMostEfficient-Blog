...
- 角标：带边框的面板与按钮内侧 5px 处画一个暗红矩形框（小元件为 4px），只在左上、右下两角额外画十字（右上、左下仅作为框角）。全图只有这两个交点颜色最深，所有线条从交点向外连续变淡、无恒定色段 —— CSS 只能画有界的线段，靠端点淡出才读得成“向两端延伸的直线”。覆盖文章卡片、上下篇、友链卡片、最近更新面板、正文阅读面、分页、标签、打印按钮、回到顶部等。`<input>`（.search-box）因伪元素不渲染、代码块与表格因自身横向滚动而排除
...
- 角标用**一个伪元素叠 8 层 `background`**（4 条框边 + 4 条十字臂）画完，不增加 DOM。关键在于**没有任何恒定色段**：十字臂用 `--cn-h / --cn-v`（中心最浓、两端淡出），框边用 `--cn-e-tl-r / --cn-e-tl-d / --cn-e-br-l / --cn-e-br-u`（四条边各从所在交点角**单向**淡出），因此全图只有左上、右下两个交点最深。若框边写成“两端淡出、中间固定色”，边线中段会一直满色，就不是“只有交点最深”。`--cn-solid` 备用。**不能用 `border` 代替背景层**：伪元素必须保持 `inset: 0`，否则 `background-clip: border-box` 会裁掉十字伸出框外的部分。定位用 calc + 百分比镜像（`--cn-span = 100% - inset × 2`，下/右再减一个线宽）。颜色 `--cn-color` 亮色暗红 `#7f1d1d` / 暗色 `#a63a34`；`--cn-inset` 内缩，`--cn-arm` / `--cn-w` 控臂长与 1px 发丝线宽。该层必须 `z-index: 1`：亚克力/光泽伪元素是 `z-index: 0` 且晚于 `::before` 绘制，不提升会被半透底盖淡；内容也是 `z-index: 1` 但晚于伪元素，文字仍在角标之上。打印时隐藏
