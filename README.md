...
- 角标：仅在**面板**内侧 5px 处画矩形框 + 左上/右下两枚十字（小元件、按钮、标签一律不加）。线条为“交点色 → 终点色”渐变，全图只有两个交点是满色：**亮色为蓝→白，暗色为暗红→黑**（终点色均接近底色，所以读起来是从交点淡出）。覆盖文章卡片、上下篇、友链卡片、最近更新面板、正文阅读面、目录面板。`<input>`（.search-box）因伪元素不渲染、代码块与表格因自身横向滚动而排除
