...
- 7 档正文字号调节
- 动态可互动背景（Canvas 粒子网络，跟随指针交互，详见[性能与可访问性](#性能与可访问性)）
- 文章目录、阅读进度、滚动定位、回到顶部
...
- 全屏 Canvas 粒子网络：粒子缓慢漂移、近距离自动连线，指针靠近时被柔和推开，点击产生扩散涟漪
- 颜色取自 CSS 变量（`--fg` 作粒子、`--accent` 作连线与光晕），随明暗主题自动切换
- 性能约束：设备像素比上限 2、粒子数按视口面积计算并有 64 上限、连线按距离分 4 档批量描边（每帧仅 4 次 `stroke`）、粒子数据使用 `Float32Array` 避免每帧产生垃圾、指针光晕使用离屏预渲染精灵
- 省电策略：页面切到后台（`visibilitychange`）或组件卸载时彻底停帧；窗口尺寸变化用 rAF 合并处理
- 无障碍：`aria-hidden` 装饰性图层、`pointer-events: none` 不拦截任何点击/选中、打印时自动隐藏
- 尊重 `prefers-reduced-motion: reduce`：只绘制一帧静态星图，不启动动画循环、不绑定指针交互

想要关闭或调参：在 `my-app/app/layout.tsx` 移除 `<InteractiveBackground />` 即可关闭；粒子数量、速度、斥力半径、涟漪时长等都在该组件顶部的常量区集中定义。
