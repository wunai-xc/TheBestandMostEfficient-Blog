...
- 四角描边（顶点最深，沿边向外逐渐变淡、变细）：带边框的面板与按钮都有，包括文章卡片、上下篇、友链卡片、最近更新面板、正文阅读面、分页、标签、打印按钮、回到顶部等；尺度分级由 `--cn-scale` 控制。`<input>`（.search-box）因伪元素不渲染、代码块与表格因自身横向滚动而排除
...
- 四角描边用**一个伪元素叠 16 层 `background`** 画完，不增加 DOM：每角每边叠“短而粗 + 长而细”两层，超出粗线只剩细线，所以向外既变淡也变细；顶点处横竖两线重叠，自然最浓。token 在 `:root` 的 `--cn-*`，颜色取 `--accent`；线长乘 `--cn-scale`，小按钮类设 0.45 以免细线长过元素本身。该层必须 `z-index: 1`：亚克力/光泽伪元素是 `z-index: 0` 且晚于 `::before` 绘制，不提升会被半透底盖淡；内容也是 `z-index: 1` 但晚于伪元素，文字仍在角标之上。打印时隐藏。代价是每元素 16 层渐变，列表页可能到二三百层，移动端光栅化开销不可忽略
