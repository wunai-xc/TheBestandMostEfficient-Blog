...
- 四角十字角标：带边框的面板与按钮在四角内侧各有一枚小金十字，交点最浓、沿臂向两端渐淡渐细；覆盖文章卡片、上下篇、友链卡片、最近更新面板、正文阅读面、分页、标签、打印按钮、回到顶部等，内缩与臂长按尺度分两级。`<input>`（.search-box）因伪元素不渲染、代码块与表格因自身横向滚动而排除
...
- 十字角标用**一个伪元素叠 8 层 `background`**（4 角 × 2 臂）画完，不增加 DOM。每条臂是“透明 → 金 → 透明”的对称渐变，中段最浓、两端淡出；两臂在中心交叠，自然形成最浓的交点。定位用 calc + 百分比（`--cn-a` 沿臂方向距边，`--cn-b` 垂直方向居中，右侧/下侧用 `calc(100% - …)` 镜像），因此元素尺寸变化仍贴角。颜色 `--cn-cross` 亮色深金 `#b08d3f` / 暗色浅金 `#e0c383`；`--cn-inset` 内缩使其离开边框，`--cn-arm` / `--cn-w` 控臂长与线宽（1px 发丝）。该层必须 `z-index: 1`：亚克力/光泽伪元素是 `z-index: 0` 且晚于 `::before` 绘制，不提升会被半透底盖淡。卡片内容也是 `z-index: 1` 但晚于伪元素，文字仍在十字之上；唯一例外是 `.post-content`（正文阅读面的十字会压在内容上，只占四角 9px 小区域）。打印时隐藏
