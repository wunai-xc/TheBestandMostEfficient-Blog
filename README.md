...
- 角标：带边框的面板与按钮内侧画一个暗红矩形框，只在左上、右下两角额外画十字（右上、左下仅作为框角）。全图只有这两个交点颜色最深，所有线条从交点向外连续变淡、无恒定色段 —— CSS 只能画有界的线段，靠端点淡出才读得成“向两端延伸的直线”。覆盖文章卡片、上下篇、友链卡片、最近更新面板、正文阅读面、分页、标签、打印按钮、回到顶部等，内缩与臂长按尺度分两级。`<input>`（.search-box）因伪元素不渲染、代码块与表格因自身横向滚动而排除
