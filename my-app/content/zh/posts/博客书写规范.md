...
- **提取前必须剥掉围栏代码块与行内代码**。否则像《Markdown 基本语法》里那张语法表（`` `![alt](url "title")` ``）会把示例文本当成真图，卡片上就会多一个坏图。
- 缩略图可能是外链，对方可能禁外链 → `<img onError>` 时隐藏整个缩略图，不留碎图占位。

布局用一个 flex：正文包在 `.post-card-body`（**必须 `min-width: 0`**，否则长标题会把缩略图挤出容器），缩略图是 `.post-card-thumb`（宽 `clamp(84px, 20vw, 132px)`、`aspect-ratio: 16/11`、`object-fit: cover`）。无缩略图时 `.post-card:not(.has-thumb)` 退回纵向排版。
