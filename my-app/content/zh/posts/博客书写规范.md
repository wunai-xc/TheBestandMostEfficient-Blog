...
带边框的**面板**，在其内侧 5px 处画一个矩形框（四条边连起来），并只在**左上、右下两角**额外画十字；右上与左下只是框线的拐角。线条为**蓝色渐变到白**：交点是纯蓝，向外渐变到白（浅色下白即底色，读起来就是“从交点淡出”）。

**颜色分布：全图只有两个交点是纯蓝**

- 十字的 4 条臂：`--cn-h / --cn-v`，白 → 蓝（交点）→ 白
- 框的 4 条边：`--cn-e-tl-r / --cn-e-tl-d`（上边与左边，从左上交点向右/向下）、`--cn-e-br-l / --cn-e-br-u`（下边与右边，从右下交点向左/向上），各自是蓝 → 白的单向渐变

> 关键：**不能有恒定色段**。早先写成“两端淡出、中间固定色”，边线中段一直是满色，就不是“只有交点最深”了；上面四组单向渐变才是正确做法。

**颜色 token**：`--cn-color`（交点，亮色 `#2563eb` / 暗色 `#3b82f6`）与 `--cn-end`（渐变终点，两主题均为 `#ffffff`）。

> ⚠️ 暗色下终点是真正的白，会在深底上亮出来（而浅色下白＝底色，看起来是淡出）。这是按要求做的两主题一致；若希望暗色也“淡出”，把 `html.dark` 里的 `--cn-end` 改成透明色即可。

**关于“线段 / 射线 / 直线”**

CSS 只能画有界的色块，因此画出来的一律是**线段**（两端点）；真正的**直线**（两端无限）与**射线**（一端无限）无法表达。让线段读起来像直线的手段就是**端点渐变到与底色相同** —— 渐变暗示线还在继续（所以十字的臂两端都渐变到白，看起来像一条穿过交点的无限长线）。

**覆盖范围**

| 元素 |
|------|
| `.post-card`、`.post-nav a`、`.friend-card`、`.changelog-list`、`.post-content`（正文阅读面）、`.collection-card`、`.toc-panel` |
...
- **四组单向/对称渐变，没有任何恒定色段**：`--cn-h / --cn-v`（十字臂，白→蓝→白）、`--cn-e-tl-* / --cn-e-br-*`（框边，各从交点角蓝→白单向渐变）；另有 `--cn-solid` 备用，想要无渐变的实线时可用。
...
- **颜色** `--cn-color`（交点）：亮色 `#2563eb` / 暗色 `#3b82f6`；`--cn-end`（渐变终点）：两主题均为 `#ffffff`。
...
调参：`--cn-color`（交点蓝）/ `--cn-end`（渐变终点白）/ `--cn-inset`（内缩）/ `--cn-arm`（臂长）/ `--cn-w`（线宽），整体浓度改伪元素上的 `opacity`（默认 0.6）。
