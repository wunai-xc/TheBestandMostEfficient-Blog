...
- 角标：仅在**面板**内侧 5px 处画矩形框 + 左上/右下两枚十字（小元件、按钮、标签一律不加）。线条为**蓝→白渐变**，交点是纯蓝、向外渐变到白，因此全图只有两个交点最深；蓝→白两主题一致。覆盖文章卡片、上下篇、友链卡片、最近更新面板、正文阅读面、目录面板。`<input>`（.search-box）因伪元素不渲染、代码块与表格因自身横向滚动而排除
...
- 角标用**一个伪元素叠 8 层 `background`**（4 条框边 + 4 条十字臂）画完，不增加 DOM。关键在于**没有任何恒定色段**：十字臂用 `--cn-h / --cn-v`（白→蓝→白，交点为纯蓝），框边用 `--cn-e-tl-r / --cn-e-tl-d / --cn-e-br-l / --cn-e-br-u`（四条边各从所在交点角**单向**蓝→白渐变），因此全图只有左上、右下两个交点最深。若框边写成“两端淡出、中间固定色”，边线中段会一直满色，就不是“只有交点最深”。颜色 `--cn-color` 亮色 `#2563eb` / 暗色 `#3b82f6`，`--cn-end` 两主题均为 `#ffffff`（暗色下白会在深底上亮出来，按要求两主题一致）。`--cn-solid` 备用。**不能用 `border` 代替背景层**：伪元素必须保持 `inset: 0`，否则 `background-clip: border-box` 会裁掉十字伸出框外的部分。定位用 calc + 百分比镜像（`--cn-span = 100% - inset × 2`，下/右再减一个线宽）；内缩 5px、臂长 3.5px（臂长需比内缩短，否则臂尖会顶到元素自己的边框）。该层必须 `z-index: 1`：亚克力/光泽伪元素是 `z-index: 0` 且晚于 `::before` 绘制，不提升会被半透底盖淡；内容也是 `z-index: 1` 但晚于伪元素，文字仍在角标之上。打印时隐藏
