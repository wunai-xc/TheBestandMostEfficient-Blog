...
- 角标：带边框的面板与按钮内侧画一个暗红矩形框，只在左上、右下两角额外画十字（右上、左下仅作为框角）；所有线都带渐变（两端淡出）—— CSS 只能画有界的线段，靠淡出才读得成“向两端延伸的直线”。覆盖文章卡片、上下篇、友链卡片、最近更新面板、正文阅读面、分页、标签、打印按钮、回到顶部等，内缩与臂长按尺度分两级。`<input>`（.search-box）因伪元素不渲染、代码块与表格因自身横向滚动而排除
...
- 角标用**一个伪元素叠 8 层 `background`**（4 条框边 + 4 条十字臂）画完，不增加 DOM。四组渐变各司其职：`--cn-h / --cn-v` 给十字臂（中段实、**两端完全淡出**），`--cn-eh / --cn-ev` 给框边（中段实、**只在靠近拐角处淡出**，否则角落断掉不成框）；`--cn-solid` 备用，想要无渐变的实线时可用。**不能用 `border` 代替背景层**：伪元素必须保持 `inset: 0`，否则 `background-clip: border-box` 会裁掉十字伸出框外的部分。定位用 calc + 百分比镜像（`--cn-span = 100% - inset × 2`，下/右再减一个线宽）。颜色 `--cn-color` 亮色暗红 `#7f1d1d` / 暗色 `#a63a34`（真正的暗红在深底上看不见）；`--cn-inset` 内缩，`--cn-arm` / `--cn-w` 控臂长与 1px 发丝线宽。该层必须 `z-index: 1`：亚克力/光泽伪元素是 `z-index: 0` 且晚于 `::before` 绘制，不提升会被半透底盖淡；内容也是 `z-index: 1` 但晚于伪元素，文字仍在角标之上。打印时隐藏
